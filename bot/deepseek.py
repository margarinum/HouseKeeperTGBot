...
SYSTEM_PROMPTS = {
    "barrier": (
        ...
        "Тебе будут предоставлены фрагменты документов. "
        "Отвечай СТРОГО на основе этих фрагментов и ТОЛЬКО по теме шлагбаума. "
        ...
        "Не используй общие знания. Не отвечай на вопросы не по теме шлагбаума. "
        "Отвечай на русском языке, кратко и по делу."
    ),
    "bot": (
        "Ты помощник по вопросам работы бота верификации жильцов. "
        "Тебе будут предоставлены фрагменты документов. "
        "Отвечай СТРОГО на основе этих фрагментов и ТОЛЬКО по теме работы бота. "
        "Если ответа нет в предоставленных фрагментах — ответь: "
        "'Извините, в документах нет информации по вашему вопросу. Обратитесь к администратору.' "
        "Не используй общие знания. Не отвечай на вопросы не по теме бота. "
        "Отвечай на русском языке, кратко и по делу."
    ),
}
...
class DeepSeekClient:
    def __init__(self, api_key: str, knowledge_base_dir: str, rag_persist_dir: str = ""):
        ...
        self._stores: dict[str, KnowledgeStore] = {
            "barrier": KnowledgeStore("barrier"),
            "bot": KnowledgeStore("bot"),
        }

    def reload_knowledge(self) -> None:
        for topic, store in self._stores.items():
    ...
    async def ask(self, topic: str, question: str) -> str:
        ...
        if not chunks:
            return "Извините, документы по данной теме ещё не загружены. Обратитесь к администратору."
